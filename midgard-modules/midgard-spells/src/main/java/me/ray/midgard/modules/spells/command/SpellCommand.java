...
public class SpellCommand extends MidgardCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("matrix")) {
            if (args.length < 2) {
                me.ray.midgard.core.text.MessageUtils.send(player, module.getMessage("commands.usage_matrix"));
                return;
            }
            String spellId = args[1];
            if (m.getSpellManager().getSpell(spellId) == null) {
                player.sendMessage("§cSpell not found: " + spellId);
                return;
            }
            new me.ray.midgard.modules.spells.gui.SpellMatrixGUI(player, m, spellId).open();
            return;
        }
        ...
        if (args[0].equalsIgnoreCase("combo")) {
            ...
            SpellProfile profile = m.getSpellManager().getProfile(player);
             if (profile == null) {
                   player.sendMessage("§cProfile not loaded.");
                   return;
                }
